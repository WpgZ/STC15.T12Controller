...
int32_t IDATA Setting[16] = {200, S_EMPTY_PASS, 249, 64, 4, 16, 41, 0, 0, 200, S_UNDEFINED, S_UNDEFINED, S_UNDEFINED, S_FREE, S_UNDEFINED, 0};
...
int32_t calcRotateValue(int32_t value, BIT cw, int32_t min, int32_t max, BIT loop, uint8_t step)
{	
	int32_t ret = value;
	if(cw){
		ret += step;
...
void onEncoderRotate(BIT cw, BIT fs)
{
	...
	case STATE_SETTING_P:
		lastParameter = calcRotateValue(lastParameter, cw, 0, SETTING_NUM-1, 1, fs?3:1);			
		break;
	case STATE_HEATING:
		{
	...
	case STATE_SETTING_V:
		Setting[lastParameter] = calcSettingValue(Setting[lastParameter], cw, fs, lastParameter);
		break;
	case STATE_EX_PREHEATING:
		exHeatingTime = calcSettingValue(exHeatingTime, cw, fs, S_TIME_EXHEATING);
		break;
	case STATE_PASS_CONFIG:
	case STATE_PASS_VERIFY:
		digits[led_idx] = calcRotateValue(digits[led_idx], cw, 0, 9, 1, 1);
		if(state == STATE_PASS_VERIFY && (digits[2]*100+digits[1]*10+digits[0]) == Setting[S_PASS]){			
			state = next_state;
...
void DoConfig(void)
{
	...
	if(encoderState != ENCODER_NONE){
		if(encoderState & ENCODER_CW_CCW)
			onEncoderRotate((encoderState & ENCODER_CW), encoderState & ENCODER_FASTSTEP);	
